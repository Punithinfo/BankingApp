    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                git 'https://github.com/Punithinfo/BankingApp.git'
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t punithinfo/bankapp-eta-app:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag punithinfo/bankapp-eta-app:${BUILD_NUMBER} loksaieta/bankapp-eta-app:latest"
            }
        }
		...
		stage('Push2DockerHub') {
			...
			steps {
				sh "docker push punithinfo/bankapp-eta-app:latest"
			}
		}
